...
int rsa_get_sk_from_file(char *file, rsa_sk_t *sk, rsa_pk_t *pk)
{
    FILE *fp;
    fp = fopen(file, "rb");
    if(fp == NULL) {
    ...
    pk->bits = sk->bits;
    memcpy((uint8_t*)pk->modulus, (uint8_t*)sk->modulus, RSA_MAX_MODULUS_LEN);
    memcpy((uint8_t*)pk->exponent, (uint8_t*)sk->public_exponet, RSA_MAX_MODULUS_LEN);
    return 0;
}

void write_sk(char* file, rsa_sk_t* sk)
{
    FILE* fp;
    fp = fopen(file, "wb");
    if (fp == NULL) {
